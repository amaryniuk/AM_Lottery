...
public class Lottery {
    ...
    static int numTickets = 50;

    // for new game
    static int ticketsSold = 0;
    static Boolean drawDone = false;
        ...
        while ((command = scanner.nextLine()) != null) {
            ...
            if ("Purchase".equalsIgnoreCase(command) || "p".equalsIgnoreCase(command)) {
                if (ticketsSold >= numTickets) {
                    System.out.println("Tickets are sold out, time to do a draw.");
                } else if (drawDone) {
                    System.out.println("Draw already done. Show winners or start a new game.");
                } else {
...
//        System.out.println(tics);
    ...
    public static void newGame() {
        System.out.println("\n\nNew Lottery Started. Available commands:\n[P] Purchase, [D] Draw, [W] Winners, [N] New Game, [V] View Tickets, [Q] Quit.");
        drawDone = false;
        ticketsSold = firstBall = secondBall = thirdBall = 0;
        firstWinner = secondWinner = thirdWinner = null;
        ticketList.clear();
